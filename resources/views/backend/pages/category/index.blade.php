...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
  ...
  <div class="ms-auto">
    <div class="btn-group">
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addCategoryModal">Add Category <i class="lni lni-plus"></i></button>
    </div>
  </div>
  @endif
</div>
...
<!-- add modal -->
@include( 'backend.components.category.add')
<!-- Modal -->
...
<script>
  $(document).ready(function() {
   ...
   // table search perpage pagination setup end
	 ...
	 // insert row
	 $(document).on('submit', '#addCategoryStore', function(e){
	      e.preventDefault();
				$('#addCategoryModal').modal('hide');
				$('.full_scren').removeClass('d-none');
	      let formData = new FormData($("#addCategoryStore")[0]);
	      $.ajax({
	 ...
	 $(document).on('submit', '#CategoryUpdate', function(e){
	      e.preventDefault();
				...
				$('.full_scren').removeClass('d-none');
	      let formData = new FormData($("#CategoryUpdate")[0]);
	      $.ajax({
	          url: "{{url('/categorie/update')}}",
	          type: 'POST',
	          data: formData,
	          processData: false,
	          contentType: false,
